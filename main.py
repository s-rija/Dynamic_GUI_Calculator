...
root.title("Ananya's Calculator")
...
screen.grid(row=0, columnspan=4, padx=12,
			pady=12, ipady=5)
...
for i in range(len(key_matrix)):
	...
	for j in range(len(key_matrix[i])):
		...
		root, bd=1, text=str(key_matrix[i][j]), font=myFont,bg='lavenderblush1')
		
		# Positioning buttons
		btn_dict["btn_"+str(key_matrix[i][j])].grid(
		row=i+1, column=j, padx=5, pady=5, ipadx=5, ipady=3)
